...
const MyAppointements = () => {
  ...
  const cancelAppointment =async(appointmentsId)=>{
      ...
      try{
         ...
         const {data}= await axios.post(backendUrl+'/api/user/cancel-appointment',{appointmentsId},{headers:{token}})
         if(data.success){
           ...
           {appointments.map((item,index)=>(
            <div className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b' key={index}>
                 ...
                 <div className='flex-1 text-sm text-zinc-600'>
                    ...
                    <p>Address:</p>
                    <p className='text-zinc-700 font-medium mt-1'>{item.docData.address.line1}</p>
                    <p className='text-x5'>{item.docData.address.line2}</p>
